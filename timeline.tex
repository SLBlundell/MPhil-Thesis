% Preamble: \usepackage{tikz} \usetikzlibrary{arrows.meta, decorations.pathreplacing}
...
% Period labels with brackets
\draw[decorate, decoration={brace, amplitude=8pt}, thick] 
    (0.6, \ytop+0.4) -- (6.2, \ytop+0.4) 
    node[periodlbl, midway, yshift=18pt] {Period 0 \;\; ($t=0$)};

\draw[decorate, decoration={brace, amplitude=8pt}, thick] 
    (7.2, \ytop+0.4) -- (12.8, \ytop+0.4) 
    node[periodlbl, midway, yshift=18pt] {Period 1 \;\; ($t=1$)};
...
\caption{Within-period sequence of events by agent, with nodes denoting actions undertaken by a respective agent type within the given sub-period. Labels (a)--(e) correspond to those above in Section~\ref{sec:timings}.}
\label{fig:timeline}
\end{figure}